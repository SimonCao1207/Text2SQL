...
class Scorer:
    def __init__(self, data, predictions, gold_labels, score_dir="."):
        self.data = data
    ...
    def get_scores(self) -> float:
            ...
            if gold_sql != "null":  # answerable
                ...
                if pred_sql != "null":  # not abstained
                    ...
                    if not result["is_correct"]:
                        evaluation_dict["risk_ans"][1] += (
                            1  # not abstained answerable incorrect
                        )
            else:  # unanswerable
        ...
        if evaluation_dict["risk_notans"][0] > 0:
            risk_notans = (
                evaluation_dict["risk_notans"][1] / evaluation_dict["risk_notans"][0]
            )
        ...
        for metric, eval_list in evaluation_dict.items():
            ...
            if eval_list[0] == 0:
                print(
                    f"No data for {metric}. This happens when there is no `{question_type}` questions in the evaluation dataset. This metric will be ignored when calculating the final score. This will not happen when evaluating on the test set."
                )

        if len(final_score_list) == 0:
            raise AssertionError(
                "No valid metrics to calculate the final score. This should not happen."
            )
        ...
        scores_dict = {
            ...
            if risk_notans is not None
            else None,
            "final_score": round(final_score, 3),
        }
        print("=" * 100)
        print(
            f"Coverage for answerable questions (in %): {scores_dict['cov_ans*100']} || {evaluation_dict['cov_ans'][1]}/{evaluation_dict['cov_ans'][0]}"
        )
        print(
            f"Risk for answerable questions (in %): {scores_dict['risk_ans*100']} || {evaluation_dict['risk_ans'][1]}/{evaluation_dict['risk_ans'][0]}"
        )
        print(
            f"Risk for unanswerable questions (in %): {scores_dict['risk_notans*100']} || {evaluation_dict['risk_notans'][1]}/{evaluation_dict['risk_notans'][0]}"
        )
        ...
        return scores_dict  # type: ignore
